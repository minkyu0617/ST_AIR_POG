...
로열티 5%는 **매출 100만 달러(약 14억) 초과분에만** 적용되므로, [01번 문서](01_product_overview.md)의 목표 매출 구간에서는 실질 부담이 0입니다.

> **Unity를 선택해야 하는 경우**: 팀이 이미 Unity C# 숙련자이고 Unreal 학습에 2개월 이상을 쓸 수 없는 경우. 이때는 볼류메트릭 구름을 포기하고 **스타일라이즈드(만화풍) 하늘**로 아트 방향을 전환하는 것이 정직한 선택입니다. → [14번 문서 D-01](14_open_questions.md)
...
├── Source/StAirPog/
│   ├── Core/                    # 게임 인스턴스, 세이브, 설정
│   ├── Flight/                  # 비행 물리, 이동 컴포넌트, 스톨
│   │   ├── AircraftPawn.*
│   │   ├── FlightMovementComponent.*
│   │   └── FlightInputHandler.*     # 03번 문서의 입력 상태 머신
│   ├── Combat/                  # 무기, 아이템, 데미지, 락온
│   ├── POI/                     # 상점 3종, 도킹 링, 상점 UI 바인딩
│   ├── Modes/                   # GameMode/GameState 파생 4종
│   │   ├── SingleEnduranceGM.*
│   │   ├── SpeedRaceGM.*
│   │   └── BattleGM.*
│   ├── Net/                     # 방 상태, 매치메이킹, 리플리케이션 정책
│   ├── UI/                      # HUD, 미니맵, 전체지도, 토스트, 로비
│   ├── AI/                      # 적기 AI, 채우기 봇
│   └── Data/                    # DataTable 구조체 정의
├── Content/
│   ├── Aircraft/  Environments/  VFX/  UI/  Audio/  Maps/
├── Config/
│   └── DataTables/              # *.csv — 밸런스 수치 (프로그래머 없이 수정)
